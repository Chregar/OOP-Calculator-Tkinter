...
class calculator:
    def __init__(self):
        ...
        self.total_expression = "" #Total expression is the expression above the current one
        self.current_expression = "" #Current expression is where you are currently typing
    ...
    def create_display_labels(self):
        total_label = tk.Label(self.display_frame,text =self.total_expression,anchor = tk.E,
                               fg = LABEL_COLOR,bg =LIGHT_GRAY,padx = 24,font = SMALL_FONT_STYLE)
        total_label.pack(expand = True,fill = "both")

        label = tk.Label(self.display_frame, text=self.current_expression, anchor=tk.E,
                               fg=LABEL_COLOR, bg=LIGHT_GRAY, padx=24, font=LARGE_FONT_STYLE)
    ...
    def create_digit_buttons(self):
        for digit,grid_value in self.digits.items():
            button = tk.Button(self.buttons_frame,text = str(digit),bg = WHITE,fg =LABEL_COLOR,
                               font = DIGITS_FONT_STYLE,borderwidth = 0,command = lambda x=digit: self.add_expression(x))
    ...
    def create_operator_buttons(self):
        ...
        for operator,symbol in self.operations.items():
            button = tk.Button(self.buttons_frame,text = symbol,bg = OFF_WHITE,fg = LABEL_COLOR,
                               font = DEFAULT_FONT_STYLE,borderwidth = 0,command = lambda x = operator
